...
class GAN():
    ...
    def __init__(self, batch_size = 100, lr_rate = 0.0002, epochs = 100, keep_prob = 0.3):
        ...
        self.fixed_z_ = np.random.normal(0, 1, (25, 100))
    ...
    def __construct_model(self):
        ...
        with tf.variable_scope('G'):
            self._z = tf.placeholder(tf.float32, shape=(None, 100))
        ...
        self._D_optim = tf.train.AdamOptimizer(self.lr_rate).minimize(self._D_loss, var_list=D_vars)
        self._G_optim = tf.train.AdamOptimizer(self.lr_rate).minimize(self._G_loss, var_list=G_vars)
    ...
    def fit(self, train_set):
        ...
        for epoch in range(self.epochs):
            ...
            for iter in tqdm(range(train_set.shape[0] // self.batch_size)):
                ...
                loss_d_, _ = self.sess.run([self._D_loss, self._D_optim], {self._x: x_, self._z: z_, self._drop_out: self.keep_prob})
                D_losses.append(loss_d_)

                # update generator
                z_ = np.random.normal(0, 1, (self.batch_size, 100))
                loss_g_, _ = self.sess.run([self._G_loss, self._G_optim], {self._z: z_, self._drop_out: self.keep_prob})
